...
# This is the original code for 3 channel mask
...
def create_patches(image_path, mask_path, output_dir, patch_size, stride):
    ...
    with rasterio.open(image_path) as src_img, rasterio.open(mask_path) as src_mask:
        ...
        # Iterate over patches using Window
        for y in range(0, src_img.height, stride):
            for x in range(0, src_img.width, stride):

                # Ensure patch doesn't exceed image bounds
                win_width = min(patch_size, src_img.width - x)
                win_height = min(patch_size, src_img.height - y)
                ...
                # Create binary mask: 1 for building, 0 for background
                binary_mask = np.where(mask_patch > 0, 1, 0).astype(np.uint8)

                # Skip empty mask patches
                if np.count_nonzero(binary_mask) == 0:
                    continue

                # Update metadata for patch
                img_patch_meta = img_meta.copy()
                img_patch_meta.update({
                    "height": win_height,
                    "width": win_width,
                ...
                # Save image patch
                image_patch_name = f"patch_{no_of_patches_saved}.tif"
                image_patch_path = os.path.join(output_dir, "images", image_patch_name)
                with rasterio.open(image_patch_path, "w", **img_patch_meta) as dst:
                    dst.write(img_patch)

                # Save mask patch
                mask_patch_name = f"patch_{no_of_patches_saved}_mask.tif"
                mask_patch_path = os.path.join(output_dir, "masks", mask_patch_name)
                with rasterio.open(mask_patch_path, "w", **mask_patch_meta) as dst:
                    dst.write(binary_mask, 1)

                no_of_patches_saved += 1

        print(f"Total patches saved: {no_of_patches_saved}")
